...
type GuestPatch = { name?: string; meal?: string; role?: string };
...
export function GuestListSheet({ objects, onUpdateGuest }: Props) {
  const rows = buildGuestRows(objects);
    ...
    <section className="guest-list-sheet flex min-h-0 flex-1 flex-col bg-[#f7f6f1]">
      <header className="flex shrink-0 items-center justify-between border-b border-[#dce2dd] bg-[#fffefa] px-7 py-5">
        <div>
          ...
          <p className="mt-1 text-xs text-[#748078]">One row per seat. Names and notes stay with the exact table or linked chair.</p>
        </div>
      ...
      <div className="subtle-scrollbar min-h-0 flex-1 overflow-auto p-6">
        {rows.length ? (
          <div className="mx-auto max-w-6xl overflow-hidden rounded-xl border border-[#d8dfda] bg-white shadow-sm">
            <table className="w-full table-fixed border-collapse text-left">
              <thead className="sticky top-0 z-10 bg-[#eaf0ec] text-[10px] font-bold uppercase tracking-[0.13em] text-[#53675b] shadow-[0_1px_0_#ccd6cf]">
                <tr>
                  <th className="w-[18%] px-4 py-3">Table number</th>
                  <th className="w-[30%] border-l border-[#d5ddd7] px-4 py-3">Guest name</th>
                  <th className="w-[24%] border-l border-[#d5ddd7] px-4 py-3">Meal / dish</th>
                  <th className="w-[28%] border-l border-[#d5ddd7] px-4 py-3">Role / group</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, rowIndex) => (
                  <tr key={`${row.objectId}:${row.seatIndex}`} className={rowIndex % 2 ? "bg-[#fbfcfa]" : "bg-white"}>
                    ...
                    />
                    <GuestCell
                      ariaLabel={`${row.tableLabel}, seat ${row.displaySeatNumber}, meal or dish`}
                      placeholder="Meal or dish"
                      value={row.meal}
...
function buildGuestRows(objects: readonly EventObject[]): GuestRow[] {
