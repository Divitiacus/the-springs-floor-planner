...
export function FloorPlanner({
  ...
  useEffect(() => {
    ...
    const receivePortalSaveRequest = (event: MessageEvent) => {
      ...
      const markFinal = (event.data as { markFinal?: unknown }).markFinal;
      if (typeof markFinal !== "boolean") return;

      const eventName = editor.layout.name.trim();
      if (!eventName) {
        editor.showNotice("This Event needs a name before the floor plan can be saved.");
        return;
      }

      const payload = JSON.parse(
        serializePortableFloorplan({
          ...editor.layout,
          name: eventName,
        }),
      ) as unknown;
      ...
      editor.showNotice(
        markFinal
          ? "Sending Final plan to your Event…"
          : "Sending plan to your Event…",
      );
    };

    window.addEventListener("message", receivePortalSaveRequest);
    return () => window.removeEventListener("message", receivePortalSaveRequest);
  }, [editor.layout, editor.ready, editor.showNotice, portalBridge]);
  ...
  if (!editor.ready) {
    return (
      <div
        className={`grid place-items-center bg-[#f6f4ef] text-sm text-[#66736c] ${
          portalBridge ? "h-[100dvh] min-h-0" : "min-h-screen"
        }`}
      >
  ...
  return (
    <main
      className={`floor-planner-shell floor-planner-view-${workspaceView} flex flex-col overflow-hidden bg-[#f6f4ef] ${
        portalBridge ? "h-[100dvh] min-h-0" : "h-screen min-h-[720px]"
      }`}
    >
      {!portalBridge ? (
